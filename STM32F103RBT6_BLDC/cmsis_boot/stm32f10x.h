...
typedef struct
{
  ...
  __IO uint32_t CR1;
  __IO uint32_t CR2;
  __IO uint32_t SMPR1; //采样时间（10~17）通道
  __IO uint32_t SMPR2; //采样时间（0~9）通道
  __IO uint32_t JOFR1; //注入通道数据偏移寄存器
  __IO uint32_t JOFR2;
  __IO uint32_t JOFR3;
  __IO uint32_t JOFR4;
  __IO uint32_t HTR;	 //看门狗高阈值寄存器
  __IO uint32_t LTR;	 //看门狗低阈值寄存器
  __IO uint32_t SQR1;	 //规则序列寄存器（规则通道序列长度及13~16通道转换）
  __IO uint32_t SQR2;	 //规则7~12通道转换
  __IO uint32_t SQR3;	 //规则1~6通道转换
  __IO uint32_t JSQR;	 //注入序列寄存器
  __IO uint32_t JDR1;	 //注入数据寄存器
  __IO uint32_t JDR2;
  __IO uint32_t JDR3;
  __IO uint32_t JDR4;
